...
def prepare_aster_code_review(
    *,
    ...
    if (base is None) == (not paths):
        raise ValueError("agent review requires exactly one of --base or one or more --path values")
    ...
    if not launcher.is_file() or not skill.is_file():
        raise ValueError("the Asterinas checkout does not contain the aster-code-review skill")
    if not profile_root.is_dir():
        choices = sorted(path.name for path in (skill_root / "agent_profiles").iterdir() if path.is_dir())
        raise ValueError(f"unknown Asterinas review profile {agent_profile!r}; choose one of: {choices}")
    ...
    handoff = {
        ...
        "profile_files": {
            path.relative_to(profile_root).as_posix(): _sha256(path) for path in profile_files
        },
...
def _validate_target(root: Path, raw: str) -> str:
    ...
    if ":" in raw:
        ...
        if possible_range and all(char.isdigit() or char in "-," for char in possible_range):
            path_part = possible_path
    path = (root / path_part).resolve()
    try:
        relative = path.relative_to(root)
    except ValueError as error:
        raise ValueError(f"agent-review path is outside Asterinas checkout: {raw}") from error
    if not path.is_file():
