...
gh pr view 14 --repo chinrw/asterinas
...
PR #23 run `33084600540` passed `validate` and the 20-target PR profile. Main
push run `33087541192` then passed both jobs on merge commit `58b07bf...`.
Manual main run `33088274665` passed the complete 24-target nightly profile.
Its downloaded Host evidence pack contained 165 files and 1,577,150 bytes;
...
timespec content revision `820ec6464809071779f3c386634befcc83da10bc` and is
reachable from `chinrw/asterinas/main`.

`chinrw/asterinas` PR #14's signed content head
`0bc8839d496f185dd7662c79d53e98619bf1169c` passed all 42 reported checks and
merged as `a20a44592fbd6e6efdf92af486873cb6f4c83bc5`. The content head is the
current aster-syssec pin and is reachable from `chinrw/asterinas/main`.
`chinrw/asterinas/main` had no branch protection at merge time, so GitHub
executed an auto-merge request immediately rather than waiting for non-required
